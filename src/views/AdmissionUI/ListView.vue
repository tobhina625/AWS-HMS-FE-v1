...
import AdmissionStatisticsDashboard from '@/components/Admission/AdmissionStatisticsDashboard.vue';
...
<template>
  <DefaultLayout>
    <ListViewTemplate :title="pageTitle" breadcrumb-title="Admissions" :loading="loading"
      :selection-count="selectionCount" :show-bulk-actions="selectionCount > 0"
      @select-all="selectAll(apiResponse.data)" @deselect-all="deselectAll">
      <template #subtitle>Manage patient admissions and discharge records.</template>

      <template #search>
        <SearchWithViewToggle v-model="viewMode" v-model:date-filter="dateFilter" :showAdd="true"
          placeholder="Search admissions..." addButtonRoute="admissions/add" @search="getSearchTerm"
      ...
      <template #table>
        ...
        <EmptyState v-if="isEmpty" title="No Admissions Found"
          description="Get started by adding your first admission record." icon="default"
          action-label="Add First Admission" @action="handleAddNew" />

        <div v-else-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 p-6">
          <GridViewCard v-for="admission in apiResponse.data" :key="admission.id" :id="admission.id"
            :title="`Admission #${admission.id}`"
            :subtitle="admission.patient ? `${admission.patient.firstName} ${admission.patient.lastName}` : 'No Patient'"
            :fields="getAdmissionFields(admission)" :selected="selectedIds.includes(admission.id)" :show-details="true"
            :show-edit="true" :show-delete="canDelete" @detail="handleView(admission)" @edit="handleEdit(admission)"
            @delete="handleDelete(admission)" @select="toggle(admission.id)" />
        </div>

        <DynamicTable v-else :data="apiResponse.data" :columns="admissionColumns" :selectable="true"
          :selected-ids="selectedIds" :status-color-map="STATUS_OPTIONS.ADMISSION_STATUS_COLORS" item-key="id"
          :showDetails="true" :showEdit="true" :showDelete="canDelete" @detail="handleView" @edit="handleEdit"
          @delete="handleDelete" @select="toggle"
          @select-all="(checked: boolean) => toggleAll(checked, apiResponse.data)" />
      </template>

      <template #pagination>
        <DynamicPagination v-if="hasData" :currentPage="listFilters.page" :totalPages="apiResponse.totalPages"
          :totalElements="apiResponse.totalElements" :itemsPerPage="apiResponse.itemsPerPage"
          :startIndex="apiResponse.startIndex" @change-page="handlePageChange"
