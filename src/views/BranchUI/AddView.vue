  ...
  const submitBranch = async () => {
    ...
    try {
      ...
      if (response.isSuccess) {
        showAlert('success', 'Branch added successfully.', 'Success');
        router.push('/branches');
      } else {
        showAlert('error', response.error || 'Failed to add branch.', 'Error');
      }
    } catch (err: any) {
      const msg = err?.response?.data?.error || err?.message || 'Failed to add branch';
...
<template>
  <DefaultLayout>
    <FormViewTemplate title="Create New Branch" breadcrumb-title="Branches" :is-submitting="isSubmitting" @submit="submitBranch" @cancel="router.back()">
      <BaseInput
        label="Branch Name"
