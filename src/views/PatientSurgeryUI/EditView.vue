<script setup lang="ts">
  ...
  const loadDetails = async () => {
    ...
    try {
      const response = await patientSurgeryService.getPatientSurgeryById(Number(route.params.id));
      if (response.data) {
        const surgeryTime = new Date(response.data.surgeryTime);
        const endTime = new Date(response.data.endTime);

        formData.value = {
          id: response.data.id,
          surgeryTime: surgeryTime.toISOString().slice(0, 16),
          endTime: endTime.toISOString().slice(0, 16),
          notes: response.data.notes || '',
          surgeryId: response.data.surgeryId,
          operationTheatreId: response.data.operationTheatreId,
          admissionId: response.data.admissionId,
        };
      }
    } catch {
      showAlert('error', 'Failed to load patient surgery details.', 'Error');
  ...
  const submitUpdate = async () => {
    ...
    try {
      const response = await patientSurgeryService.updatePatientSurgery({
      ...
      if (response.isSuccess) {
        showAlert('success', 'Patient surgery updated successfully.', 'Success');
        router.push('/patient-surgeries');
      } else {
        showAlert('error', response.error || 'Failed to update patient surgery.', 'Error');
      }
    } catch (error: any) {
      const message = error?.response?.data?.error || error?.message || 'An unexpected error occurred while updating the patient surgery.';
...
<template>
  <DefaultLayout>
    ...
    <FormViewTemplate
      v-else
      title="Edit Patient Surgery"
      breadcrumb-title="Patient Surgeries"
      :breadcrumb-slug="`Surgery #${formData.id}`"
