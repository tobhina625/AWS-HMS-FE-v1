<template>
  ...
  >
    ...
    <!-- Calendar Navigation -->
    <div class="flex items-center text-lg justify-between mb-4 border-b pb-2 overflow-hidden">
  ...
  <div class="flex gap-2 overflow-x-auto max-w-full">
    <div
      ...
      :key="index"
      :class="[ 
        'px-3 py-2 rounded-lg cursor-pointer whitespace-nowrap',
      ...
      @click="selectDay(day)"
    >
      <div class="text-lg font-medium text-center">{{ day.day }}</div>
...
<div class="space-y-4 max-h-96 overflow-y-auto dark:border-black">
  <div
    ...
    <div class="flex-1">
      ...
      <div class="flex items-center text-sm text-gray-600 mt-1">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-6 mr-1 ">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6l4 2m8-6a10 10 0 11-20 0 10 10 0 0120 0z" />
        </svg>
        <span>{{ appointment.time }}</span>
        <span class="ml-2">$ {{ appointment.cost }}</span>
      </div>
    </div>
    <!-- Buttons -->
